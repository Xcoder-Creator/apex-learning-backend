...
const del = require('del'); //Import del module

const delete_archived_class = async (req, res) => {
                                                        ...
                                                        if (result.status === false){
                                                            ...
                                                            if (result.data.length > 0 && result.data.length === 1){
                                                                ...
                                                                if (result.status === false){
                                                                    ...
                                                                    if (result.data.length > 0 && result.data.length === 1){
                                                                        let result = await db_query.check_if_teacher_of_class_and_archived(user_id, class_code);
                                                                        ...
                                                                        if (result.status === false){
                                                                            ...
                                                                            if (result.data.length > 0 && result.data.length === 1){
                                                                                let result1 = await db_query.delete_archived_class(class_code);

                                                                                if (result1.status === false){
                                                                                    res.statusCode = 500;
                                                                                    res.json({ status: 'error_occured' });
                                                                                } else if (result1.status === true){
                                                                                    let result1 = await db_query.delete_all_joined_classes(class_code);

                                                                                    if (result1.status === false){
                                                                                        res.statusCode = 500;
                                                                                        res.json({ status: 'error_occured' });
                                                                                    } else if (result1.status === true){
                                                                                        let result1 = await db_query.delete_all_attached_files(class_code);

                                                                                        if (result1.status === false){
                                                                                            res.statusCode = 500;
                                                                                            res.json({ status: 'error_occured' });
                                                                                        } else if (result1.status === true){
                                                                                            let result1 = await db_query.delete_all_class_posts(class_code);

                                                                                            if (result1.status === false){
                                                                                                res.statusCode = 500;
                                                                                                res.json({ status: 'error_occured' });
                                                                                            } else if (result1.status === true){
                                                                                                let result1 = await db_query.delete_all_private_comments(class_code);

                                                                                                if (result1.status === false){
                                                                                                    res.statusCode = 500;
                                                                                                    res.json({ status: 'error_occured' });
                                                                                                } else if (result1.status === true){
                                                                                                    let result1 = await db_query.delete_all_public_comments(class_code);

                                                                                                    if (result1.status === false){
                                                                                                        res.statusCode = 500;
                                                                                                        res.json({ status: 'error_occured' });
                                                                                                    } else if (result1.status === true){
                                                                                                        let result2 = await db_query.delete_classworks_for_a_class(class_code);

                                                                                                        if (result2.status === false){
                                                                                                            res.statusCode = 500;
                                                                                                            res.json({ status: 'error_occured' });
                                                                                                        } else if (result2.status === true){
                                                                                                            let result3 = await db_query.delete_classwork_response(class_code);
                                                                                                            ...
                                                                                                            if (result3.status === false){
                                                                                                                ...
                                                                                                                (async () => {
                                                                                                                    try {
                                                                                                                        await del(`./public/class_folders/${class_code}`)
                                                                                                                    } catch (err){
                                                                                                                        ////console..log(err);
                                                                                                                    }
                                                                                                                })();
                                                                                                                ...
                                                                                                                res.json({ status: 'class_deleted_successfully', token_info: 'token_available', new_accessToken: new_access_token });
                                                                                                            }
                                                                                                        }
                                                                                                    }
                                                                                                }
                                                            ...
                                                            if (result.status === false){
                                                                ...
                                                                if (result.data.length > 0 && result.data.length === 1){
                                                                    ...
                                                                    if (result.status === false){
                                                                        ...
                                                                        if (result.data.length > 0 && result.data.length === 1){
                                                                            let result = await db_query.check_if_teacher_of_class_and_archived(user_auth_id, class_code);
                                                                                                        ...
                                                                                                        if (result1.status === false){
                                                                                                            ...
                                                                                                            if (result2.status === false){
                                                                                                                ...
                                                                                                                if (result3.status === false){
                                                                                                                    ...
                                                                                                                    res.statusCode = 200;
                                                                                                                    res.json({ status: 'class_deleted_successfully', token_info: 'no_token' });
                                                                                                                }
                                                                                                            }
                                                                                                        }
                                                                                                    }
...
module.exports = delete_archived_class;