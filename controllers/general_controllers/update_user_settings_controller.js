...
const update_user_settings = async (req, res) => {
    ...
    if (environment === 'development'){
        ...
        if (req.body){
            ...
            if (form_data.access_token && form_data.option && form_data.state){
                //Filter and sanitize the request parameters
                let access_token = sanitize_data(form_data.access_token);
                let option = parseInt(form_data.option, 10);
                let state = sanitize_data(form_data.state);
                                                                ...
                                                                if (result.status === false){
                                                                    ...
                                                                    if (result.data.length > 0 && result.data.length === 1){
                                                                        ...
                                                                        if (user_role === 'Student'){
                                                                            if (option === 1){
                                                                                let resultx = await db_query.update_settings_for_user(option, state, user_id);
                                                                                ...
                                                                                if (resultx.status === false){
                                                                                    ...
                                                                                    res.json({ status: 'settings_updated', token_info: 'token_available', new_accessToken: new_access_token });
                                                                                }
                                                                            } else {
                                                                                res.statusCode = 401;
                                                                ...
                                                                if (status === 'user_direct_access_token'){
                                                                    ...
                                                                    if (result.status === false){
                                                                        ...
                                                                        if (result.data.length > 0 && result.data.length === 1){
                                                                            ...
                                                                            if (user_role === 'Student'){
                                                                                if (option === 1){
                                                                                    let resultx = await db_query.update_settings_for_user(option, state, user_auth_id);
                                                                                    ...
                                                                                    if (resultx.status === false){
                                                                                        ...
                                                                                        res.statusCode = 200;
                                                                                        res.json({ status: 'settings_updated', token_info: 'no_token' });
                                                                                    }
                                                                                } else {
                                                                                    res.statusCode = 401;
                                                                                    res.json({ status: 'unauthorized' });
        ...
        if (req.body){
            ...
            //Check if the appropriate request parameters are set
            if (form_data.option && form_data.state){
                //Filter and sanitize the request parameters
                let option = parseInt(form_data.option, 10);
                let state = sanitize_data(form_data.state);
...
module.exports = update_user_settings;